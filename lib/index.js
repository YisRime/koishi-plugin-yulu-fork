...
async function getfileByUrl(url, fileName, dir) {
  try {
    const stream = fsSync.createWriteStream((0, import_path.join)(dir, fileName));
    return new Promise((resolve2, reject) => {
      const request = url.startsWith("https:") ? import_https.request : import_http.request;
...
function apply(ctx, cfg) {
  ...
  __name(getQuotedId, "getQuotedId");
  async function getQuotedYuluId(session, ctx2) {
    try {
      ...
      if (session.quote?.messageId || session.quote?.id) {
        ...
        const yuluRecords = await ctx2.database.get("yulu", { origin_message_id: messageId });
  ...
  __name(getQuotedYuluId, "getQuotedYuluId");
  try {
  ...
  ctx.command("yulu/yulu_remove [...rest]").alias("删除语录").option("id", "-i <id:number>").action(async ({ session, options }) => {
    const removeYulu = /* @__PURE__ */ __name(async (id) => {
      const finds = await ctx.database.get("yulu", id);
      if (finds.length == 0) {
        return session.text(".no-result");
      }
      try {
        const filePath = (0, import_path.join)(cfg.dataDir, String(id));
  ...
  ctx.command("yulu/yulu_add [...rest]").alias("添加语录").action(async ({ session }, ...rest) => {
  ...
  ctx.command("yulu/yulu_tag_add [...rest]").alias("addtag").action(async ({ session }, ...rest) => {
    if (rest.length < 1) {
  ...
  ctx.command("yulu/yulu_tag_remove [...rest]").alias("rmtag").action(async ({ session }, ...rest) => {
    if (rest.length === 0) {
  ...
  ctx.command("yulu/yulu_select [...rest]").alias("语录").option("id", "-i <id:number>").option("global", "-g").option("tag", "-t").option("list", "-l").option("page", "-p <page:number>").option("full", "-f").shortcut("引用语录", { fuzzy: true, options: { global: true } }).action(async ({ session, options }, ...rest) => {
    ...
    if (options.id) {
      ...
      if (!options.list) {
        ...
        if (ctx.cache) {
          await ctx.cache.set(`yulu_recent_send_${session.guildId}`, String(find.id), true, 1e3 * 500);
        }
        return res;
      } else {
        let page;
